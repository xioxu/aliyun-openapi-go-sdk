...
package rds_DescribeTasksResponse
...
func (m *DescribeTasksResponse) BuildProperties(parentKey string, flatObj map[string]interface{}) {

	m.Items = make([]TaskProgressInfo, 0)
	for i := 0; i < (flatObj[parentKey+".Items.length"]).(int); i++ {
		tmp := TaskProgressInfo{}
