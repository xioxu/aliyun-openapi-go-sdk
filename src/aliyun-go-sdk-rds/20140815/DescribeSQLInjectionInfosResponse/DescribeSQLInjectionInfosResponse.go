...
package rds_DescribeSQLInjectionInfosResponse
...
func (m *DescribeSQLInjectionInfosResponse) BuildProperties(parentKey string, flatObj map[string]interface{}) {

	m.Items = make([]SQLInjectionInfo, 0)
	for i := 0; i < (flatObj[parentKey+".Items.length"]).(int); i++ {
		tmp := SQLInjectionInfo{}
