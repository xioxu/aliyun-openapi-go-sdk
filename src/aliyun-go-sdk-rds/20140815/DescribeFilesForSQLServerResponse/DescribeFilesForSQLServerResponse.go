...
package rds_DescribeFilesForSQLServerResponse
...
func (m *DescribeFilesForSQLServerResponse) BuildProperties(parentKey string, flatObj map[string]interface{}) {

	m.Items = make([]SQLServerUploadFile, 0)
	for i := 0; i < (flatObj[parentKey+".Items.length"]).(int); i++ {
		tmp := SQLServerUploadFile{}
