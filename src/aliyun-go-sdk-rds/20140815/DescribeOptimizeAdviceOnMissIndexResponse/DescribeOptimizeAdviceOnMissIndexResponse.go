...
package rds_DescribeOptimizeAdviceOnMissIndexResponse
...
func (m *DescribeOptimizeAdviceOnMissIndexResponse) BuildProperties(parentKey string, flatObj map[string]interface{}) {

	m.Items = make([]AdviceOnMissIndex, 0)
	for i := 0; i < (flatObj[parentKey+".Items.length"]).(int); i++ {
		tmp := AdviceOnMissIndex{}
