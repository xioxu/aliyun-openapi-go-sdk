...
type OnsClusterListResponse struct {
	Data      []ClusterInfoDataDo
	HelpUrl   string
	RequestId string
}

func (m *OnsClusterListResponse) BuildProperties(parentKey string, flatObj map[string]interface{}) {

	m.Data = make([]ClusterInfoDataDo, 0)
	for i := 0; i < (flatObj[parentKey+".Data.length"]).(int); i++ {
		tmp := ClusterInfoDataDo{}
