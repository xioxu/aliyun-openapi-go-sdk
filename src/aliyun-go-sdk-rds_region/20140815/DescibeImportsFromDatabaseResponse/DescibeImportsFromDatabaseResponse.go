...
func (m *DescibeImportsFromDatabaseResponse) BuildProperties(parentKey string, flatObj map[string]interface{}) {

	m.Items = make([]ImportResultFromDB, 0)
	for i := 0; i < (flatObj[parentKey+".Items.length"]).(int); i++ {
		tmp := ImportResultFromDB{}
