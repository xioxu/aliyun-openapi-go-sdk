...
func (m *DescribeModifyParameterLogResponse) BuildProperties(parentKey string, flatObj map[string]interface{}) {

	m.Items = make([]ParameterChangeLog, 0)
	for i := 0; i < (flatObj[parentKey+".Items.length"]).(int); i++ {
		tmp := ParameterChangeLog{}
