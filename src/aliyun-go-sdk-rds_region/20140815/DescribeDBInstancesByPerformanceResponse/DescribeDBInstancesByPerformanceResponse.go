...
package rds_region_DescribeDBInstancesByPerformanceResponse
...
func (m *DescribeDBInstancesByPerformanceResponse) BuildProperties(parentKey string, flatObj map[string]interface{}) {

	m.Items = make([]DBInstancePerformance, 0)
	for i := 0; i < (flatObj[parentKey+".Items.length"]).(int); i++ {
		tmp := DBInstancePerformance{}
