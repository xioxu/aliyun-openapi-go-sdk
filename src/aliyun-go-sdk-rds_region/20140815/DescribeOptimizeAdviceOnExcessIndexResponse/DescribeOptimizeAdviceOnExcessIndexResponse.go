...
package rds_region_DescribeOptimizeAdviceOnExcessIndexResponse
...
func (m *DescribeOptimizeAdviceOnExcessIndexResponse) BuildProperties(parentKey string, flatObj map[string]interface{}) {

	m.Items = make([]AdviceOnExcessIndex, 0)
	for i := 0; i < (flatObj[parentKey+".Items.length"]).(int); i++ {
		tmp := AdviceOnExcessIndex{}
