...
package rds_region_DescribeOptimizeAdviceOnBigTableResponse
...
func (m *DescribeOptimizeAdviceOnBigTableResponse) BuildProperties(parentKey string, flatObj map[string]interface{}) {

	m.Items = make([]AdviceOnBigTable, 0)
	for i := 0; i < (flatObj[parentKey+".Items.length"]).(int); i++ {
		tmp := AdviceOnBigTable{}
